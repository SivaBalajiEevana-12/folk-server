...
const adminArtOfMindModel = require("../Models/Admin.artControl.model")
const mongoose = require("mongoose");
const artController ={
    getData: async(req,res)=>{
       try {
        const data =await  artModel.find().sort({ createdAt: -1 });
    ...
    getAllEventsByLocation: async (req, res) => {
      try {
        const data = await adminArtOfMindModel.aggregate([
          {
            $lookup: {
              from: "arts", 
              localField: "_id",
  ...
  updateAttendance: async (req, res) => {
   try {
     ...
     const user = await artModel.findById(userId);
     if (!user) {
       return res.status(404).send({ message: "User not found" });
     }

     const event = await adminArtOfMindModel.findById(eventId);
     if (!event) {
